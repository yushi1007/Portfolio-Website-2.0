...
//Reassigning value inside the component
  ...
  return (
    <div id='projects' ref={slideRef} className='text-white flex justify-center items-center lg:bg-cover lg:bg-center lg:bg-no-repeat py-40 overflow-hidden'>
          ...
          <div>
            ...
            <div className='px-14 lg:px-36 py-[24vh] absolute z-10 bottom-0 w-full max-w-[92vw] lg:max-w-none'>
              ...
              <div className='flex flex-row gap-4 group-hover:translate-y-[200%] transition-transform duration-500 mt-4'>
                <div>
                  { checkLink(projects[currentIndex].frontend) ? <a href={projects[currentIndex].frontend} className='transition-all duration-300 py-1 px-1 text-blue-dark bg-indigo-600 hover:bg-indigo-800 font-semibold flex items-center space-x-1 rounded' target="_blank" rel="noreferrer">
                    <DiGithubAlt size={20} />
                    <span>Frontend</span>
                  </a>:null}
                </div>
                <div>
                  { checkLink(projects[currentIndex].backend) ? <a href={projects[currentIndex].backend} className='transition-all duration-300 py-1 px-1 text-blue-dark bg-indigo-600 hover:bg-indigo-800 font-semibold flex items-center space-x-1 rounded' target="_blank" rel="noreferrer">
                    <DiGithubAlt size={20} />
                    <span>Backend</span>
                  </a>:null}
                </div>
                <div>
                  { checkLink(projects[currentIndex].live) ? <a href={projects[currentIndex].live} className='transition-all duration-300 py-1 px-1 text-blue-dark bg-indigo-600 hover:bg-indigo-800 font-semibold flex items-center space-x-1 rounded' target="_blank" rel="noreferrer">
                    <CgWebsite size={20} />
                    <span>Live</span>
                  </a>:null}
                </div>
                <div>
                  { checkLink(projects[currentIndex].video) ? <a href={projects[currentIndex].video} className='transition-all duration-300 py-1 px-1 text-blue-dark bg-indigo-600 hover:bg-indigo-800 font-semibold flex items-center space-x-1 rounded' target="_blank" rel="noreferrer">
                    <MdLiveTv size={20} />
                    <span>Video</span>
                  </a>:null}
                </div>
                <div>
                  { checkLink(projects[currentIndex].repo) ? <a href={projects[currentIndex].repo} className='transition-all duration-300 py-1 px-1 text-blue-dark bg-indigo-600 hover:bg-indigo-800 font-semibold flex items-center space-x-1 rounded' target="_blank" rel="noreferrer">
                    <DiGithubAlt size={20} />
                    <span>Repo</span>
                  </a>:null}
                </div>
